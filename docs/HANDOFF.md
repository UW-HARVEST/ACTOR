...
| 24 | **make the reproducibility job actually pass in CI** | — | **DONE as #118** (`1b1e70a`). **CI IS GREEN**: run 32227106630 on `main` is the first passing `validate-translations` since before 2026-07-05, and it genuinely replays — 85 translate hits, 85 seeding hits, 85 verify hits, `0 agent invocation(s)` on all three tally lines, `[verified] 85/85 (393v)` matching the stored record, `.eval/` empty, and the committed `tables/results.md` row reproduced exactly. Two causes, neither guessable: `CliVersion::probe` runs `<agent> --version` PER CASE, so a runner with no `claude` failed before the store was consulted (`HARVEST_CLI_VERSION` is the documented hatch; `cli` left the key in #109 and a replay stores nothing, so it moves no key); and the cache commit was INCOMPLETE — `results/.gitignore` excluded `Cargo.lock` and `**/.cargo/`, both `StoreAndHash`, so 268 files were missing and a fresh clone quarantined **all 175 entries** on the digest check. Fixed in `results` `e31aec03e`. A `--replay-only` miss now prints its derived components once, which is what proved the key was IDENTICAL on both machines and ruled out the key hypothesis after two wasted CI rounds |
| 26 | **four batteries validated in CI from the cache; tables republished** | — | **DONE as #120** (`3a70820`). Campaign earned 240 cache entries (175 -> 415, `results` `44498c17e`); CI matrix now B01_synthetic, B01_organic, B02_organic, P00_perlin_noise, all green on the real runner with `0 run` / `0 agent invocation(s)`. Tables regenerated: **B02_synthetic 36/42 -> 40/42**, total **319 -> 323/338**, LOC/unsafe% move on every claude row. Every claude `summary.json` had been written 2026-07-20 against 2026-08-04 data; the 27 dead `api_error` runs that blocked three batteries are now zero. Fixes a bug I shipped in #119: `tractor.tex`'s columns are Builds and Tests of the HEADLINE phase, not translated vs verified, so that check compared translated-cases against a builds count — it passed only where they coincide and broke on B02_organic (44 compile, 43 pass). Now compares headline cases+vectors against `results.md`. **Excluded:** P01_sphincs_plus (IS one shared-source group, `Mode::Bypass`, nothing in the store) and B02_synthetic (headline reproduces, but each replay re-propagates its group crate and leaves the three `macrodepth_*` followers' `result.json` enrichment stale — spec-7c's path) |
...
it was single-tool, named a flag (`--tool`) and a results layout that no longer exist, and every
