...
# tool can be diffed byte for byte. CI splits by tool for wall clock, so a subset run instead requires
# every row it regenerated to appear VERBATIM in the committed table; three jobs cover every row.
...
if grep -q 'Tables regenerated' "$log"; then
  ...
  if [ "$TABLES" = identical ]; then
   ...
   nothing produces that file any more."
    ...
    # A subset run: rows for the other tools are absent, and every row it DID produce must be the
    # committed one character for character.
    for f in $written; do
