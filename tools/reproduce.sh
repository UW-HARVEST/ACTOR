...
# PR #116 removes the eval tree even when the score exits 1; a tree left standing is one the next
# run could read.
...
# cannot be scored until their dead runs are re-run, and every claude summary.json was last written
# 2026-07-20 against case data from 2026-08-04. A whole-file diff fails for staleness this run
...
got = re.search(rf"{re.escape(battery)} \[verified\]: (\d+)/(\d+) cases, (\d+)/(\d+) vectors",
                open(scored).read())
if not got:
    sys.exit(f"no [verified] score for {battery} in the run's output — nothing to compare")
measured = (f"{got[1]}/{got[2]}", f"{got[3]}/{got[4]}")
...
for line in open(table):
    if line.startswith("## "):
        ...
        break
if row is None:
    sys.exit(f"the committed table has no `{agent}` row under `## {battery}`")
...
print(f"✅ {battery} [{agent}] reproduces the published table row exactly")
PY
