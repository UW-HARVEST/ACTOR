...
# tool can be diffed byte for byte -- a subset run rewrites the file from its own rows and blanks the
# rest. CI splits by tool for wall clock, so a subset run instead requires every row it regenerated to
# appear VERBATIM in the committed table. Three such jobs cover every row between them.
...
# Which tables THIS run regenerated, taken from what it said it wrote rather than from a list here:
# `git diff` alone cannot tell "regenerated the same bytes" from "never regenerated", and a list would
# be a second definition of which dataset writes which file.
...
if grep -q 'Tables regenerated' "$log"; then
  ...
  if [ "$TABLES" = identical ]; then
   ...
   above, then commit it deliberately if the new numbers are the ones you mean to publish."
    ...
    # Every committed table must have been REGENERATED, not merely left alone: a file nothing produces
    # any more would otherwise pass for ever on stale numbers.
   ...
   nothing produces that file any more."
    ...
    # $TOOLS is a subset, so rows for the other tools are absent from what was just written. Every row
    # this run DID produce still has to be the committed one, character for character.
    for f in $written; do
      missing=$(grep -vE '^%|^$' "$f" | while IFS= read -r line; do
        git show "HEAD:$f" | grep -qxF "$line" || echo "$line"
      done)
      [ -z "$missing" ] || die "$f: $TOOLS regenerated row(s) that are not in the committed table:
$missing"
    done
    echo "✅ every row $TOOLS regenerated is verbatim in the committed tables"
  fi
else
