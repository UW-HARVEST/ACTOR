...
/// PROOF that provenance was established, and the stamp to write into every artifact.
///
/// The reason this is a value and not a free function: `harness_id` had exactly one caller -- the
/// warning string inside `require_reproducible` that promises "Artifacts will be stamped
/// `<sha>-dirty`". Nothing wrote it anywhere. `result.json` carried battery/case/loc/passed/unsafe,
/// `summary.json` six counts, `AgentRecord` outcome/tree/secs/cost/cli, and `tables/` a bare
/// "GENERATED by" line -- not one had a commit field. So an edited tree run with `--allow-dirty`
/// produced artifacts byte-indistinguishable from a clean run's, which is precisely the confusion the
/// message claims to prevent and the rule CLAUDE.md prices at $625 and twelve hours: "An artifact
/// records what produced it".
///
/// Obtainable only from [`require_reproducible`], and required by every artifact writer, so an artifact
/// cannot be written without the check having run.
...
impl Provenance {
    ...
    /// For tests that need an artifact writer without going through a git repository.
    #[cfg(test)]
...
pub fn require_reproducible(on_problem: OnUnreproducible) -> Result<Provenance> {
    let head = head_sha();
    let dirty = dirty_file_count().unwrap_or(0);
    match assess(built_from(), head.as_deref(), dirty) {
        None => Ok(Provenance(harness_id())),
        Some(problem) => {
