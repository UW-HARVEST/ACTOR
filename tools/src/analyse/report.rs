...
fn generate_numbers_tex(rows: &[BatteryRow], repo_root: &Path, kiro_dir: Option<&str>) -> String {
    ...
    // claim that it passed nothing -- which was false in print: `\ActorKiroTests` published `0/338`
    // for an agent whose own committed records say 325/338, and `\ActorCodexTests` `0/338` against
    // 243/338. Both because a claude-only run attests claude only, so `unwrap_or(0)` answered for
    // every other agent. Same rule `HarvestBenchRow` already states: absent metrics print `--`,
