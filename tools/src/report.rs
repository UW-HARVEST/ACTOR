...
fn laertes_vs_c2rust(repo_root: &Path) -> LaertesDelta {
    ...
    for (case, c2_ok) in &c2 {
        if let Some(&la_ok) = la.get(case) {
            if *c2_ok && !la_ok { d.broke += 1; }
            if !*c2_ok && la_ok { d.fixed += 1; }
        }
    }
    d
}
...
fn kiro_cost(base: &Path) -> KiroCost {
    ...
    let Ok(rd) = std::fs::read_dir(base) else { return zero() };
...
fn aggregate_cases_phase(
    ...
    let Ok(entries) = std::fs::read_dir(bat_dir) else {
        return CaseTotals { total_loc: 0, unsafe_lines: 0, cases_built: 0 };
    };
    ...
    CaseTotals { total_loc, unsafe_lines: total_unsafe, cases_built: built }
}
