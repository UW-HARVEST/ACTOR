...
fn resolve() -> Result<PathBuf> {
    ...
    resolve_from(
        ...
        if std::env::var_os(ENV_ALLOW_TMPFS).is_some() { Tmpfs::Allow } else { Tmpfs::Refuse },
        &mounts,
    )
}
...
";
    ...
    fn default_base_derives_from_home_and_is_not_tmp_or_cache() {
        let tmp = tempfile::tempdir().unwrap();
        let home = tmp.path().join("home");
        let got = resolve_from(None, Some(home.clone()), Tmpfs::Allow, MOUNTS).expect("resolves from HOME");
    ...
    fn missing_home_without_override_errors_and_says_what_to_set() {
        let err = resolve_from(None, None, Tmpfs::Allow, MOUNTS).expect_err("no base and no HOME must fail");
        let err = format!("{err:#}");
