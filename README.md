...
2. Translate it with any agent. The agent works only from the C source in
   ...
   ```bash
   # Agentic translation with Claude Code (or --tool kiro)
   harvest-tools --tool claude translate mycases/hello

   # Any model via OpenCode — here Claude Sonnet 5 on Amazon Bedrock
   harvest-tools --tool opencode \
     --model amazon-bedrock/us.anthropic.claude-sonnet-5 \
     translate mycases/hello

   # Mechanical transpilation with c2rust
   harvest-tools --tool c2rust translate mycases/hello
   ```
...
| **kiro-translate** | Derived column (not an `--tool`) | kiro's pre-verify result — its `translated/` phase scored without the verify/repair loop (the "no validate" column). Reported for every agent automatically; no separate run. |
...
`--tool opencode` (alias `oc`) runs the *same* `prompts/claude/*.md` prompts
and the same translate → verify → score pipeline as `--tool claude`, so its
...
C LOC counted from `test-corpus/Public-Tests/`. Regenerated by `harvest-tools --tool <a> run all`,
